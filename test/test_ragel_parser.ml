...
let print_token_stream stream =
  let print_token (_, token) _throw k =
    print_endline @@ String.escaped @@ token_to_string token;
    k ()
  in
  print_endline "tokens:";
  Kstream.iter print_token stream (wrong_k "failed") ignore

let print_signal_stream stream =
  let print_token (_, token) _throw k =
    print_endline @@ String.escaped @@ signal_to_string token;
    k ()
  in
  print_endline "tokens:";
  Kstream.iter print_token stream (wrong_k "failed") ignore
...
let expect ?prefix ?(context = Some `Document) text signals =
  ...
  let signals =
    ...
    | _, _, E (`Bad_token _) | _, _, E (`Bad_document "doctype should be first")
      -> false
    | _ -> true) signals
  in
  ...
  in
  (* print_signal_stream signal_stream; *)
  iter iterate signal_stream;
  ended ()
  ...
  [
    ( "html.parser.basic" >:: fun _ ->
      ...
      expect ~prefix:true " <!--foo--> <!DOCTYPE html>"
        [ (1, 2, S (`Comment "foo")); (1, 13, S doctype) ];
    ...
    ( "html.parser.end-before-html" >:: fun _ ->
      expect ~prefix:true "</p><html></html>"
        [ (1, 1, E (`Unmatched_end_tag "p")); (1, 5, S (start_element "html")) ]
    );
    ...
    ( "html.parser.attributes" >:: fun _ ->
      expect "<div :class='foo'></div>"
        [
          ...
          ( 1,
            1,
            S (`Start_element ((html_ns, "div"), [ (("", ":class"), "foo") ]))
          );
          (1, 19, S `End_element);
          (1, 25, S `End_element);
          (1, 25, S `End_element);
    ...
    ( "html.parser.links" >:: fun _ ->
      expect {|<a href="foo.com?bar=on&acte=123">foo</a>|}
        [
          (1, 1, S (start_element "html"));
          (1, 1, S (start_element "head"));
          (1, 1, S `End_element);
          ...
          ( 1,
            ...
            S
              (`Start_element
                ((html_ns, "a"), [ (("", "href"), "foo.com?bar=on&acte=123") ]))
          );
          (1, 35, S (`Text [ "foo" ]));
          (1, 38, S `End_element);
          (1, 42, S `End_element);
      ...
      expect {|<a href="foo.com?bar=on&image=on">foo</a>|}
        [
          (1, 1, S (start_element "html"));
          (1, 1, S (start_element "head"));
          (1, 1, S `End_element);
          ...
          ( 1,
            ...
            S
              (`Start_element
                ((html_ns, "a"), [ (("", "href"), "foo.com?bar=on&image=on") ]))
          );
          (1, 35, S (`Text [ "foo" ]));
          (1, 38, S `End_element);
          (1, 42, S `End_element);
      ...
      expect {|<a href="foo.com?bar=on&image;">foo</a>|}
        [
          (1, 1, S (start_element "html"));
          (1, 1, S (start_element "head"));
          (1, 1, S `End_element);
          ...
          ( 1,
            ...
            S
              (`Start_element
                ((html_ns, "a"), [ (("", "href"), "foo.com?bar=onℑ") ])) );
          (1, 33, S (`Text [ "foo" ]));
          (1, 36, S `End_element);
          (1, 40, S `End_element);
          (1, 40, S `End_element);
    ...
    ( "html.parser.fragment" >:: fun _ ->
      expect
        ~context:(Some (`Fragment "title"))
        "</p>"
        [ (1, 1, S (`Text [ "</p>" ])) ];

      expect
        ~context:(Some (`Fragment "textarea"))
        "</p>"
        [ (1, 1, S (`Text [ "</p>" ])) ];
    ...
    ( "html.parser.fragment.rawtext" >:: fun _ ->
      expect
        ~context:(Some (`Fragment "style"))
        "&nbsp;</p>"
        [ (1, 1, S (`Text [ "&nbsp;</p>" ])) ] );
    ( "html.parser.fragment.script" >:: fun _ ->
      expect
        ~context:(Some (`Fragment "script"))
        "&nbsp;</p>"
        [ (1, 1, S (`Text [ "&nbsp;</p>" ])) ] );
    ( "html.parser.fragment.plaintext" >:: fun _ ->
      expect
        ~context:(Some (`Fragment "plaintext"))
        "&nbsp;</p></plaintext>"
    ...
    ( "html.parser.foreign-context" >:: fun _ ->
      expect ~context:None "<g/>"
        [
          (1, 1, S (`Start_element ((svg_ns, "g"), []))); (1, 1, S `End_element);
        ] );
    ( "html.parser.context-disambiguation" >:: fun _ ->
      expect
        ~context:(Some (`Fragment "svg"))
        "<a></a>"
        [
          (1, 1, S (`Start_element ((svg_ns, "a"), []))); (1, 4, S `End_element);
        ] );
    ...
    ( "html.parser.large-text" >:: fun _ ->
      with_text_limit 8 (fun () ->
          expect ~context:None "foobar" [ (1, 1, S (`Text [ "foobar" ])) ];
          expect ~context:None "foobarbaz"
            [ (1, 1, S (`Text [ "foobarba"; "z" ])) ]) );
    ...
    ( "html.parser.adoption-agency.stray" >:: fun _ ->
      expect ~context:None "foo</b>bar"
        [
          (1, 4, E (`Unmatched_end_tag "b")); (1, 1, S (`Text [ "foo"; "bar" ]));
        ] );
    ...
    ( "html.parser.noscript.bad" >:: fun _ ->
      expect ~context:None
        "<head><noscript><!DOCTYPE \
         html><html><head><noscript></head></noscript>"
        [
          (1, 1, S (start_element "head"));
          (1, 7, S (start_element "noscript"));
    ...
    ( "html.parser.head.fragment" >:: fun _ ->
      expect ~context:None "<base>"
        [ (1, 1, S (start_element "base")); (1, 1, S `End_element) ];

      expect ~context:None "<basefont>"
        [ (1, 1, S (start_element "basefont")); (1, 1, S `End_element) ];

      expect ~context:None "<bgsound>"
        [ (1, 1, S (start_element "bgsound")); (1, 1, S `End_element) ];

      expect ~context:None "<link>"
        [ (1, 1, S (start_element "link")); (1, 1, S `End_element) ];

      expect ~context:None "<meta>"
        [ (1, 1, S (start_element "meta")); (1, 1, S `End_element) ];

      expect ~context:None "<noframes></noframes>"
        [ (1, 1, S (start_element "noframes")); (1, 11, S `End_element) ];

      expect ~context:None "<style></style>"
        [ (1, 1, S (start_element "style")); (1, 8, S `End_element) ] );
    ( "html.parser.body.fragment" >:: fun _ ->
      expect ~context:None "<body></body>"
        [ (1, 1, S (start_element "body")); (1, 14, S `End_element) ] );
    ...
    ( "html.parser.input" >:: fun _ ->
      expect ~context:None "<input type='text'>"
        [
          ( 1,
            1,
            S (`Start_element ((html_ns, "input"), [ (("", "type"), "text") ]))
          );
          (1, 1, S `End_element);
        ] );
    ...
    ( "html.parser.generic-tag" >:: fun _ ->
      expect ~context:None "<foo></foo>"
        [ (1, 1, S (start_element "foo")); (1, 6, S `End_element) ] );
    ...
    ( "html.parser.option" >:: fun _ ->
      expect ~context:None "<option></option>"
        [ (1, 1, S (start_element "option")); (1, 9, S `End_element) ] );
    ...
    ( "html.parser.form" >:: fun _ ->
      expect ~context:None "<form></form>"
        [ (1, 1, S (start_element "form")); (1, 7, S `End_element) ] );
    ...
    ( "html.parser.frameset.fragment" >:: fun _ ->
      expect ~context:None "<frameset></frameset>"
        [ (1, 1, S (start_element "frameset")); (1, 11, S `End_element) ];

      expect ~context:None "<frame>"
        [ (1, 1, S (start_element "frame")); (1, 1, S `End_element) ] );
