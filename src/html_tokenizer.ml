...
let add_doctype_char buffer c =
  let buffer =
    match buffer with None -> Buffer.create 32 | Some buffer -> buffer
  in
  add_utf_8 buffer c;
  Some buffer
...
let tokenize report (input, get_location) =
  ...
  and emit_eof () = emit (get_location (), `EOF) (fun () -> !ended ())
  and emit_tag l tag' =
    let rec rev_deduplicate accumulator seen attributes k =
      ...
      | (n, v) :: more ->
          if list_mem_string n seen then
            report l
              (`Bad_token (n, "tag", "duplicate attribute"))
              !throw
              (fun () -> rev_deduplicate accumulator seen more k)
        ...
        (fun k ->
          if tag'.start then (
            ...
            (fun k ->
              ...
              | (n, _) :: _ ->
                  report l
                    (`Bad_token (n, "tag", "end tag with attributes"))
                    !throw k
              | _ -> k ())
            @@ (fun k () ->
                 if tag.Token_tag.self_closing then
                   report l
                     (`Bad_token
                       ("/>", "tag", "end tag cannot be self-closing"))
                     !throw k
                 else k ())
            @@ fun () -> k (`End tag)) (fun token -> emit (l, token) data_state))
  ...
  and consume_character_reference in_attribute additional location k =
    peek_option input !throw (function
      | Some (_, (0x0009 | 0x000A | 0x000C | 0x0020 | 0x003C | 0x0026)) | None
        ->
          k None
          ...
          let finish_digits prefix text s =
            ...
            let convert s semicolon k' =
              ...
              | None ->
                  report location
                    (`Bad_token
                      ( prefix ^ text ^ semicolon,
                        "character reference",
                        "out of range" ))
                    !throw
                    (fun () -> k (Some (`One u_rep)))
            in

            consume_semicolon (fun semicolon ->
                convert s semicolon (fun n' ->
            ...
            | None ->
                is_entity_like (function
                  ...
                  | Some s ->
                      report location
                        (`Bad_token
                          ("&" ^ s ^ ";", "entity reference", "no such entity"))
                        !throw
                        (fun () -> k None))
          ...
          let rec match_named best matched replace trie text =
            next_option input !throw (function
              ...
              | Some ((_, c) as v) -> (
                  ...
                  | Trie.No -> finish best matched (v :: replace)
                  | Trie.Prefix ->
                  ...
                  | Trie.Multiple m ->
                      ...
                      match_named
                        (Some (w, m))
                        (v :: (replace @ matched)) [] trie text
                  | Trie.Yes m ->
                      let w = Buffer.contents text in
                      finish (Some (w, m)) (v :: (replace @ matched)) []))
          in
          match_named None [] []
            (Lazy.force named_entity_trie)
            (Buffer.create 16))
  (* 8.2.4.1. *)
  and data_state () =
    next_option input !throw (function
      | Some (l, 0x0026) -> character_reference_state data_state l
      | Some (l, 0x003C) -> tag_open_state l
      | Some (l, 0) ->
          report l
            (`Bad_token ("U+0000", "content", "null"))
            !throw
            (fun () -> emit (l, `Char 0) data_state)
      | None -> emit_eof ()
      | Some (l, c) -> emit (l, `Char c) data_state)
  (* 8.2.4.2, 8.2.4.4. *)
  and character_reference_state state l =
    consume_character_reference false None l (function
      | None -> emit (l, `Char 0x0026) state
      | Some (`One c) -> emit (l, `Char c) state
      | Some (`Two (c, c')) ->
          emit (l, `Char c) (fun () -> emit (l, `Char c') state))
  (* 8.2.4.3. *)
  and rcdata_state () =
    next_option input !throw (function
      | Some (l, 0x0026) -> character_reference_state rcdata_state l
      | Some ((l, 0x003C) as v) ->
      ...
      | Some (l, 0) ->
          report l
            (`Bad_token ("U+0000", "content", "null"))
            !throw
            (fun () -> emit (l, `Char u_rep) rcdata_state)
      | None -> emit_eof ()
      | Some (l, c) -> emit (l, `Char c) rcdata_state)
  (* 8.2.4.5. *)
  and rawtext_state () =
    next_option input !throw (function
      | Some ((l, 0x003C) as v) ->
      ...
      | Some (l, 0) ->
          report l
            (`Bad_token ("U+0000", "content", "null"))
            !throw
            (fun () -> emit (l, `Char u_rep) rawtext_state)
      | None -> emit_eof ()
      | Some (l, c) -> emit (l, `Char c) rawtext_state)
  (* 8.2.4.6. *)
  and script_data_state () =
    next_option input !throw (function
      | Some ((l, 0x003C) as v) -> script_data_less_than_sign_state l [ v ]
      | Some (l, 0) ->
          report l
            (`Bad_token ("U+0000", "content", "null"))
            !throw
            (fun () -> emit_character l u_rep script_data_state)
      | None -> emit_eof ()
      | Some (l, c) -> emit_character l c script_data_state)
  (* 8.2.4.7. *)
  and plaintext_state () =
    next_option input !throw (function
      | Some (l, 0) ->
          report l
            (`Bad_token ("U+0000", "content", "null"))
            !throw
            (fun () -> emit (l, `Char u_rep) plaintext_state)
      | None -> emit_eof ()
      | Some (l, c) -> emit (l, `Char c) plaintext_state)
  (* 8.2.4.8. *)
  and tag_open_state l' =
    ...
    next_option input !throw (function
      | Some (_, 0x0021) -> markup_declaration_open_state l'
      | Some (_, 0x002F) -> end_tag_open_state l' tag
      ...
      | Some (_, 0x003F) ->
          report l'
            (`Bad_token
              ("<?", "content", "HTML does not have processing instructions"))
            !throw
            (fun () -> bogus_comment_state l')
      | Some ((l, c) as v) ->
          report l
            (`Bad_token (char c, "tag", "invalid start character"))
            !throw
            (fun () ->
              push input v;
    ...
    next_option input !throw (function
      ...
      | None ->
          report (get_location ()) (`Unexpected_eoi "tag") !throw (fun () ->
              ...
              emit
                (l', `Char 0x003C)
                (fun () -> emit ((line, column + 1), `Char 0x002F) data_state))
      | Some (l, c) ->
          report l
            (`Bad_token (char c, "tag", "invalid start character"))
            !throw
  ...
  and tag_name_state l' tag =
    next_option input !throw (function
      ...
      | Some (_, 0x002F) -> self_closing_start_tag_state l' tag
      | Some (_, 0x003E) -> emit_tag l' tag
      | Some (l, 0) ->
          report l
            (`Bad_token ("U+0000", "tag name", "null"))
            !throw
            (fun () ->
  ...
  and text_less_than_sign_state state l' cs =
    next_option input !throw (function
      | Some ((_, 0x002F) as v) -> text_end_tag_open_state state l' (v :: cs)
      | maybe_v ->
          push_option input maybe_v;
  ...
  and script_data_less_than_sign_state l' cs =
    next_option input !throw (function
      ...
      | Some ((_, 0x0021) as v) ->
          emit_characters
            (List.rev (v :: cs))
            (fun () -> script_data_escape_start_state l')
      | maybe_v ->
          push_option input maybe_v;
          emit_characters cs script_data_state)
  (* 8.2.4.20. *)
  ...
  and script_data_escaped_state l' =
    next_option input !throw (function
      | Some (l, 0x002D) ->
          emit_character l 0x002D (fun () -> script_data_escaped_dash_state l')
      | Some ((l, 0x003C) as v) ->
          script_data_escaped_less_than_sign_state l' l [ v ]
      | Some (l, 0) ->
          report l
            (`Bad_token ("U+0000", "script", "null"))
            !throw
            (fun () ->
              emit_character l u_rep (fun () -> script_data_escaped_state l'))
      | None ->
          report (get_location ()) (`Unexpected_eoi "script") !throw data_state
      | Some (l, c) ->
          emit_character l c (fun () -> script_data_escaped_state l'))
  (* 8.2.4.23. *)
  ...
  and script_data_escaped_dash_dash_state l' =
    next_option input !throw (function
      ...
      | Some ((l, 0x003C) as v) ->
          script_data_escaped_less_than_sign_state l' l [ v ]
      | Some (l, 0x003E) -> emit_character l 0x003E script_data_state
      | Some (l, 0) ->
          report l
            (`Bad_token ("U+0000", "script", "null"))
            !throw
    ...
    next_option input !throw (function
      | Some ((_, 0x002F) as v) ->
          text_end_tag_open_state
            (fun () -> script_data_escaped_state l')
            l'' (v :: cs)
      | Some ((_, c) as v) when is_alphabetic c ->
          ...
          emit_characters
            (List.rev (v :: cs))
            (fun () -> script_data_double_escape_start_state l' tag_buffer)
      | maybe_v ->
          push_option input maybe_v;
          emit_characters cs (fun () -> script_data_escaped_state l'))
  (* 8.2.4.28. *)
  ...
  and script_data_double_escaped_state l' =
    next_option input !throw (function
      ...
      | None ->
          report (get_location ()) (`Unexpected_eoi "script") !throw data_state
      | Some (l, c) ->
          emit_character l c (fun () -> script_data_double_escaped_state l'))
  (* 8.2.4.30. *)
  ...
  and before_attribute_name_state l' tag =
    ...
    next_option input !throw (function
      ...
      | Some (l, 0) ->
          report l
            (`Bad_token ("U+0000", "attribute name", "null"))
            !throw
            (fun () -> start_attribute u_rep)
      | Some (l, ((0x0022 | 0x0027 | 0x003C | 0x003D) as c)) ->
          report l
            (`Bad_token (char c, "attribute name", "invalid start character"))
            !throw
            (fun () -> start_attribute c)
      | Some (_, c) -> start_attribute (to_lowercase c))
  (* 8.2.4.35. *)
  and attribute_name_state l' tag name_buffer =
    next_option input !throw (function
      ...
      | Some (l, ((0x0022 | 0x0027 | 0x003C) as c)) ->
          report l
            (`Bad_token (char c, "attribute name", "invalid name character"))
            !throw
            (fun () ->
              add_utf_8 name_buffer c;
              attribute_name_state l' tag name_buffer)
  ...
  and after_attribute_name_state l' tag name =
    ...
    next_option input !throw (function
      ...
      | Some (_, 0x002F) ->
          tag.attributes <- (name, "") :: tag.attributes;
          self_closing_start_tag_state l' tag
      | Some (_, 0x003D) -> before_attribute_value_state l' tag name
      | Some (_, 0x003E) ->
          tag.attributes <- (name, "") :: tag.attributes;
          emit_tag l' tag
      | Some (l, 0) ->
          report l
            (`Bad_token ("U+0000", "attribute name", "null"))
            !throw
            (fun () -> start_next_attribute u_rep)
      | Some (l, ((0x0022 | 0x0027 | 0x003C) as c)) ->
          report l
            (`Bad_token (char c, "attribute name", "invalid start character"))
            !throw
            (fun () -> start_next_attribute c)
      | None ->
          report (get_location ()) (`Unexpected_eoi "tag") !throw data_state
      | Some (_, c) -> start_next_attribute (to_lowercase c))
  (* 8.2.4.37. *)
  and before_attribute_value_state l' tag name =
    let start_value state maybe_c =
      let value_buffer = Buffer.create 32 in
      (match maybe_c with None -> () | Some c -> add_utf_8 value_buffer c);
    ...
    next_option input !throw (function
      ...
      | Some (l, 0) ->
          report l
            (`Bad_token ("U+0000", "attribute value", "null"))
            !throw
            (fun () -> start_value attribute_value_unquoted_state (Some u_rep))
      | Some (l, 0x003E) ->
          report l
            (`Bad_token (">", "tag", "expected attribute value after '='"))
            !throw
            (fun () ->
              tag.attributes <- (name, "") :: tag.attributes;
              emit_tag l' tag)
      | Some (l, ((0x003C | 0x003D | 0x0060) as c)) ->
          report l
            (`Bad_token (char c, "attribute value", "invalid start character"))
            !throw
            (fun () -> start_value attribute_value_unquoted_state (Some c))
      | None ->
          report (get_location ()) (`Unexpected_eoi "tag") !throw data_state
      | Some (_, c) -> start_value attribute_value_unquoted_state (Some c))
  ...
  and attribute_value_unquoted_state l' tag name value_buffer =
    next_option input !throw (function
      ...
      | Some (l, ((0x0022 | 0x0027 | 0x003C | 0x003D | 0x0060) as c)) ->
          report l
            (`Bad_token (char c, "attribute value", "invalid character"))
            !throw
            (fun () ->
              add_utf_8 value_buffer c;
              attribute_value_unquoted_state l' tag name value_buffer)
  ...
  and character_reference_in_attribute allowed l value_buffer k =
    consume_character_reference true (Some allowed) l (function
      | None ->
  ...
  and after_attribute_value_quoted_state l' tag =
    next_option input !throw (function
      ...
      | Some ((l, c) as v) ->
          push input v;
          report l
            (`Bad_token (char c, "tag", "expected whitespace before attribute"))
            !throw
            (fun () -> before_attribute_name_state l' tag))
  (* 8.2.4.43. *)
  ...
  and bogus_comment_state l' =
    ...
    let rec consume () =
      next_option input !throw (function
        | Some (_, 0x003E) -> emit_comment l' buffer
        | Some (_, 0) ->
            add_utf_8 buffer u_rep;
            consume ()
        | None -> emit_comment l' buffer
        | Some (_, c) ->
            add_utf_8 buffer c;
            consume ())
    in
          ...
          peek_n 7 input !throw (fun l ->
              ...
              | [
               ...
               (_, 0x65);
                  ...
                  next_n 7 input !throw (fun _ -> doctype_state l')
              | _ ->
  ...
  and comment_start_state l' buffer =
    next_option input !throw (function
      | Some (_, 0x002D) -> comment_start_dash_state l' buffer
      | Some (l, 0) ->
          report l
            (`Bad_token ("U+0000", "comment", "null"))
            !throw
            (fun () ->
              add_utf_8 buffer u_rep;
      ...
      | Some (_, 0x003E) ->
          report l'
            (`Bad_token ("<!-->", "comment", "'-->' overlaps '<!--'"))
            !throw
            (fun () -> emit_comment l' buffer)
      | None ->
  ...
  and comment_start_dash_state l' buffer =
    next_option input !throw (function
      | Some (_, 0x002D) -> comment_end_state l' buffer
      | Some (l, 0) ->
          report l
            (`Bad_token ("U+0000", "comment", "null"))
            !throw
  ...
  and comment_state l' buffer =
    next_option input !throw (function
      | Some (_, 0x002D) -> comment_end_dash_state l' buffer
      | Some (l, 0) ->
          report l
            (`Bad_token ("U+0000", "comment", "null"))
            !throw
  ...
  and comment_end_state l' buffer =
    next_option input !throw (function
      ...
      | Some (l, 0x0021) ->
          report l
            (`Bad_token ("--!", "comment", "'--' should be in '-->'"))
            !throw
            (fun () -> comment_end_bang_state l' buffer)
      | Some (l, 0x002D) ->
          report l
            (`Bad_token ("---", "comment", "'--' should be in '-->'"))
            !throw
      ...
      | Some (l, c) ->
          report l
            (`Bad_token ("--" ^ char c, "comment", "'--' should be in '-->'"))
            !throw
            (fun () ->
              Buffer.add_string buffer "--";
              add_utf_8 buffer c;
  ...
  and doctype_state l' =
    ...
    next_option input !throw (function
      ...
      | Some ((l, c) as v) ->
          report l
            (`Bad_token (char c, "doctype", "expected whitespace"))
            !throw
            (fun () ->
              push input v;
              before_doctype_name_state l' doctype))
  ...
  and before_doctype_name_state l' doctype =
    next_option input !throw (function
      ...
      | Some (l, 0x003E) ->
          report l
            (`Bad_token (">", "doctype", "expected name"))
            !throw
            (fun () -> emit_doctype ~quirks:true l' doctype)
      | None ->
          report (get_location ()) (`Unexpected_eoi "doctype") !throw (fun () ->
              emit_doctype ~quirks:true l' doctype)
      | Some (_, c) ->
      ...
      | Some ((l'', c) as v) ->
          ...
          next_n 6 input !throw (fun l ->
              ...
              | vs ->
                  ...
                  report l''
                    (`Bad_token
                      (char c, "doctype", "expected 'PUBLIC' or 'SYSTEM'"))
                    !throw
                    (fun () ->
    ...
    next_option input !throw (function
      ...
      | Some (l, ((0x0022 | 0x0027) as c)) ->
          report l
            (`Bad_token (char c, "doctype", "expected whitespace"))
            !throw
            (fun () -> begin_public_identifier c l' doctype)
      | Some (l, 0x003E) ->
          report l
            (`Bad_token (">", "doctype", "expected public identifier"))
            !throw
            (fun () -> emit_doctype ~quirks:true l' doctype)
      | None ->
          report (get_location ()) (`Unexpected_eoi "doctype") !throw (fun () ->
  ...
  and before_doctype_public_identifier_state l' doctype =
    next_option input !throw (function
      ...
      | Some (l, c) ->
          report l
            (`Bad_token (char c, "doctype", "public identifier must be quoted"))
            !throw
            (fun () ->
              doctype.force_quirks <- true;
              bogus_doctype_state l' doctype))
  ...
  and doctype_identifier_quoted_state add quote next_state l' doctype =
    next_option input !throw (function
      | Some (_, c) when c = quote -> next_state l' doctype
      | Some (l, 0) ->
          report l
            (`Bad_token ("U+0000", "doctype", "null"))
            !throw
      ...
      | Some (l, 0x003E) ->
          report l
            (`Bad_token (">", "doctype", "'>' in identifier"))
            !throw
            (fun () -> emit_doctype ~quirks:true l' doctype)
      | None ->
          report (get_location ()) (`Unexpected_eoi "doctype") !throw (fun () ->
    ...
    next_option input !throw (function
      ...
      | Some (l, ((0x0022 | 0x0027) as c)) ->
          report l
            (`Bad_token (char c, "doctype", "expected whitespace"))
            !throw
            (fun () -> begin_system_identifier c l' doctype)
      | None ->
          report (get_location ()) (`Unexpected_eoi "doctype") !throw (fun () ->
              emit_doctype ~quirks:true l' doctype)
      | Some (l, c) ->
          report l
            (`Bad_token (char c, "doctype", "system identifier must be quoted"))
            !throw
            (fun () ->
              doctype.force_quirks <- true;
              bogus_doctype_state l' doctype))
  ...
  and after_doctype_system_keyword_state l' doctype =
    next_option input !throw (function
      ...
      | Some (l, 0x003E) ->
          report l
            (`Bad_token (">", "doctype", "expected system identifier"))
            !throw
            (fun () -> emit_doctype ~quirks:true l' doctype)
      | None ->
          report (get_location ()) (`Unexpected_eoi "doctype") !throw (fun () ->
  ...
  and after_doctype_system_identifier_state l' doctype =
    next_option input !throw (function
      ...
      | Some (l, c) ->
          report l
            (`Bad_token (char c, "doctype", "junk after system identifier"))
            !throw
            (fun () -> bogus_doctype_state l' doctype))
  (* 8.2.4.67. *)
  and bogus_doctype_state l' doctype =
    next_option input !throw (function
      | Some (_, 0x003E) -> emit_doctype l' doctype
      | None -> emit_doctype l' doctype
      | _ -> bogus_doctype_state l' doctype)
  (* 8.2.4.68. *)
  and cdata_section_state () =
    next_option input !throw (function
      ...
      | Some (l, 0x005D) ->
          peek_n 2 input !throw (function
            | [ (_, 0x005D); (_, 0x003E) ] ->
                next_n 2 input !throw (fun _ -> data_state ())
            | _ -> emit (l, `Char 0x005D) cdata_section_state)
      | Some (l, c) -> emit (l, `Char c) cdata_section_state)
  in
